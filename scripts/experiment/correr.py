...
def segundos_tope(n: int, por_minuto: float) -> int:
    return int((n * 60 + por_minuto - 1) // por_minuto) + MARGEN_S
...
def _esperar_salida(ruta: Path, proceso: subprocess.Popen[Any], tope_s: float) -> None:
    ...
    if not ruta.is_file():
        _terminar(proceso)
        raise SystemExit(f"FALLO: Locust no escribió {ruta}")
    ...
    def __call__(self, etiqueta: str, n: int, salida: Path, modo: str) -> None:
        ...
        if not mantener_ui:
            ...
            if not salida.is_file():
                raise SystemExit(f"FALLO: Locust no escribió {salida}")
            return
        if salida.is_file():
            salida.unlink()
        proceso = subprocess.Popen(comando, cwd=RAIZ, env=entorno)
        try:
            _esperar_salida(salida, proceso, float(segundos + 60))
            print(
